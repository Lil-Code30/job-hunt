...
import { subscribeToJobs, updateJobStatus, deleteJob } from '@/lib/firebase/jobs'
...
export function useJobs(filters?: JobFilters) {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter((job) => {
      ...
      if (filters.tags?.length && !filters.tags.some((t) => job.tags.includes(t))) return false
  ...
  const stats = useMemo(() => ({
    ...
    responseRate: jobs.length
      ? Math.round(
          (jobs.filter((j) => !['bookmarked', 'applied'].includes(j.status)).length /
            jobs.filter((j) => j.status !== 'bookmarked').length) *
            100
        )
      : 0,
  }), [jobs])

  async function changeStatus(jobId: string, status: Job['status'], note?: string) {
  ...
  return { jobs: filteredJobs, allJobs: jobs, loading, error, stats, changeStatus, removeJob }
}
